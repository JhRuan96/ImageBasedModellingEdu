...
FundamentalMatrix fundamental_8_point (math::Matrix<double, 3, 8> const& points1
    ...
    // 对A进行SVD分解，得到最小特征值对应的特征向量
    math::Matrix<double, 8, 9> U;
    math::Matrix<double, 9, 9> S, V;
    math::matrix_svd<double, 8, 9>(A, &U, &S, &V);
    math::Vector<double, 9> f = V.col(8);
